        ...
        <div className="max-w-[1920px] mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Status Indicator (只读，显示当前选中模型状态) */}
              {currentPage === 'trader' && status && (
                <div
                  className="flex items-center gap-2 px-3 py-2 rounded"
                  style={!(status.is_paused ?? false)
                ...
                >
                  <div
                    className={`w-2 h-2 rounded-full ${!(status.is_paused ?? false) ? 'pulse-glow' : ''}`}
                    style={{ background: !(status.is_paused ?? false) ? '#0ECB81' : '#F6465D' }}
                  />
                  <span className="font-semibold mono text-xs">
                    {t(!(status.is_paused ?? false) ? 'running' : 'stopped', language)}
                  </span>
                </div>
              )}
              {/* Peak Hour Indicator */}
...
function TraderDetailsPage({
  ...
  const handleCloseAllPositions = async () => {
    ...
    // 确认对话框
    const confirmMessage = t('confirmCloseAll', language, { traderName: selectedTrader.trader_name });
    ...
    setIsClosing(true);
    try {
      await api.closePositions(selectedTrader.trader_id);
      alert(t('closeSuccess', language));
      // 刷新页面数据
      window.location.reload();
    } catch (error: any) {
      const errorMessage = t('closeFailed', language, { error: error.message });
      alert(errorMessage);
    } finally {
      setIsClosing(false);
    }
            ...
            {positions && positions.length > 0 && (
              <>
                ...
                <button
                  ...
                  disabled={isClosing}
                  className="px-4 py-2 rounded text-sm font-bold transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                  style={{ 
                    background: isClosing ? '#5E6673' : 'linear-gradient(135deg, #F6465D 0%, #D84053 100%)', 
                    color: '#FFF',
                  ...
                  onMouseEnter={(e) => {
                    if (!isClosing) {
                      e.currentTarget.style.background = 'linear-gradient(135deg, #D84053 0%, #C0384A 100%)';
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isClosing) {
                      e.currentTarget.style.background = 'linear-gradient(135deg, #F6465D 0%, #D84053 100%)';
                    }
                  }}
                >
                  {isClosing 
                    ? t('closing', language)
                    : t('closeAllPositions', language)
                  }
                </button>
              </>
            )}
