...
`, scanIntervalMin, barIntervalMin, barCount, barIntervalMin, coverageMin, scanIntervalMin, scanIntervalMin))
	...
	sb.WriteString("- 价格在区间中部不是自动 no_trade：如果属于 trend_pullback reclaim、breakout 后第一次回踩确认、failed_breakout 回到区间内，且止损明确、净 RR 达标，可以小仓位执行。\n\n")
	...
	sb.WriteString("- 区间中部通常没有好 RR；但 trend_pullback 的 reclaim、breakout 后第一次回踩、failed_breakout 收回区间，都可以发生在中部附近。判断重点是结构失效位是否明确、净 RR 是否达标。\n")
